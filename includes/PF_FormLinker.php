...
class PFFormLinker {
	...
	static function getFormsThatPagePointsTo( $page_name, $page_namespace, $form_connection_type ) {
		...
		$form_names = PFValuesUtils::getSMWPropertyValues( $store, $subject, $prop_smw_id );
	...
	static function getFormEditLinkForPage( $target_page_title, $page_name, $page_namespace ) {
		...
		if ( count( $default_forms ) > 0 ) {
			$form_edit_url = $fe_url . "/" . $default_forms[0] . "/" . PFUtils::titleURLString( $target_page_title );
		} else {
			$form_edit_url = $fe_url;
			$form_edit_url .= ( strpos( $form_edit_url, "?" ) ) ? "&" : "?";
			$form_edit_url .= 'target=' . urlencode( PFUtils::titleString( $target_page_title ) );
		}
	...
	static function formEditLink( $title, $incoming_properties ) {
		...
		if ( '' === $namespace_name ) {
			...
			$namespace_name = wfMessage( 'pf_blank_namespace' )->inContentLanguage()->text();
		}
