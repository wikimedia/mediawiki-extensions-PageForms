...
END;
	...
	*/
	 ...
	 * 
	 * This delivers the PHP 5.3 functionality also for 5.2
	 * Pretty ugly.
	 * 
	 * Taken from http://djomla.blog.com/2011/02/16/php-versions-5-2-and-5-3-get_called_class/
	 * 
	...
	function get_called_class( $backtrace = false, $level = 1 ) {
		...
		if ( !isset( $backtrace[$level]['type'] ) ) {
			...
			switch ( $backtrace[$level]['type'] ) {
				...
				// won't get here.
				case '->': switch ( $backtrace[$level]['function'] ) {
						case '__get':
